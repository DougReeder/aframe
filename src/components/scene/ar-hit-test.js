...
module.exports.Component = register('ar-hit-test', {
  schema: {
    button: { default: 'select' },   // or 'squeeze'
  ...
  init: function () {
    if (utils.device.isMobile()) {
      this.data.button = 'select';   // no 'grip'
    }
    this.hitTest = null;
    ...
    this.el.sceneEl.renderer.xr.addEventListener('sessionstart', function () {
      ...
      session.addEventListener(this.data.button + 'start', function (e) {
      ...
      session.addEventListener(this.data.button + 'end', function (e) {
